...
public class ControllerUserLogin {
	...
	*/
	...
	protected static void doLogin(Stage ts) {
		...
		// check size at login since we are matching to an existing username
		if(username.length() > 16) {
			ViewUserLogin.label_LengthError.setText(
				"ERROR: Username is too Long, must not exceed 16 characters."
					);
			return;
		}
		ViewUserLogin.label_LengthError.setText("");
    	...
    	User user = new User(username, password, theDatabase.getCurrentFirstName(), 
    			...
    			theDatabase.getStudentRole(), theDatabase.getInstructorRole(), theDatabase.getStaffRole());
