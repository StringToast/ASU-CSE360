	...
	*/
	...
	protected static void doCreateUser() {
		...
		if(!errorMessage.isEmpty()) {
			ViewNewAccount.label_Username_Error.setText(errorMessage);
			return;
		}
		...
		if (ViewNewAccount.text_Password1.getText().
			...
			if (ViewNewAccount.theRole.compareTo("Admin") == 0) {
				roleCode = 1;
				user = new User(username, password, "", "", "", "", "", true, false, false, false, false, false);
			} else if (ViewNewAccount.theRole.compareTo("Role1") == 0) {
				roleCode = 2;
				user = new User(username, password, "", "", "", "", "", false, true, false, false, false, false);
			} else if (ViewNewAccount.theRole.compareTo("Role2") == 0) {
				roleCode = 3;
				user = new User(username, password, "", "", "", "", "", false, false, true, false, false, false);
			} else {
				System.out.println(
